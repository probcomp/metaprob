...
console.log("Being included!")
var drawTrace = function(svgId, trace, w, h) {
	var root = d3.hierarchy(trace).sort((a, b) => (a.height - b.height) || (a.name || "").localeCompare(b.name || ""))
	var svg = d3.select("#"+svgId).style("overflow-x", "scroll").style("max-width", h+100),
		width = +svg.attr("width"),
		height = +svg.attr("height"),
		g = svg.append("g")
	var tree = d3.tree().size([w, h]).separation((a, b) => 3)
	...
	var node = g.selectAll(".node")
		...
		.enter().append("g")
			.attr("class", "node")
			.attr("transform", d => "translate(" + d.y + "," + (d.x - 12) + ")")
	node.append("rect")
		.attr("height", 25)
		.attr("width", 90)
		.style("fill", "#ccc")
