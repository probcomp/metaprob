  ...
  (let [view-name    (str "view" (gensym))
        ...
        (gen []
          (let [cluster-idx (at cluster-addr categorical cluster-probs)
                ...
                _           (map
                             (fn [v] (at (str "cluster-for-" (name v))
                                         exactly cluster-idx))
                             var-names)]
            (map (fn [v]
                   (apply-at v
     ...
     (with-custom-proposal-attached
       ...
       (fn [observations]
         (gen []
           (let [score-cluster  (fn [idx]
                                  (let [new-obs  (trace-set-value
                                                  observations
                                                  cluster-addr
                                                  idx)
                                        ...
                                        ;; untraced.
                                        [_ _ s]  (infer-and-score
                                                  :procedure sampler
                                                  :observation-trace new-obs)]
                                    s))

                 cluster-scores (map score-cluster (range (count cluster-probs)))
                 chosen-cluster (at cluster-addr log-categorical cluster-scores)]
...
(defn clusters
  [& args]
  [(take-nth 2 args) (take-nth 2 (rest args))])
