(ns metaprob.distributions
  ...
  (:require [clojure.core :as clojure]
            [metaprob.prelude :as mp :refer [apply map replicate]]
...
(def uniform-discrete
  (make-primitive
   (fn [items] (nth items (mp/floor (* (mp/sample-uniform) (count items)))))
   (fn [item [items]]
...
(defn normalize-numbers [nums]
  (let [total (clojure/reduce + nums)] (map #(/ % total) nums)))
  ...
  (make-primitive
   (fn [probs]
     (if (map? probs)
       ...
       (let [total (clojure/reduce + probs)
...
(defn generate-gaussian [mu sigma]
  (+ mu (* sigma (mp/sqrt (* -2 (mp/log (mp/sample-uniform)))) (Math/cos (* 2 Math/PI (mp/sample-uniform))))))
(defn standard-gaussian-log-density [x] (* -0.5 (+ (mp/log (* 2 Math/PI)) (* x x))))
(defn score-gaussian [x [mu sigma]]
  (- (standard-gaussian-log-density (/ (- x mu) sigma)) (mp/log sigma)))
