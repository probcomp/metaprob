(ns metaprob.inference
  ...
  (:require [metaprob.distributions :as mpd]
            [metaprob.prelude :as mp]
            [metaprob.trace :as mpt]
            #?(:clj [metaprob.generative-functions :as mpg :refer [gen]])
            #?(:cljs [metaprob.generative-functions :as mpg]))
...
(defn rejection-sampling
  ...
  (let [[_ candidate-trace score]
        (mpg/infer-and-score :procedure model
                             :inputs inputs
    ...
    (cond
      ...
      log-bound (if (< (mp/log (mpd/uniform 0 1)) (- score log-bound))
                  candidate-trace
...
(defn importance-sampling
  ...
  (let [particles (mp/replicate
                   n-particles
                   (fn []
                     (let [[v t s] (mpg/infer-and-score :procedure model
                                                        :observation-trace observation-trace
                                                        :inputs inputs)]
                       [(* (mp/exp s)
                           (if f (f t) v)
                           s)])))
        normalizer (mp/exp (mpd/logsumexp (map second particles)))]
...
(defn importance-resampling
  ...
  (let [particles (replicate n-particles
                             (fn []
                               (let [[_ t s] (mpg/infer-and-score :procedure model
                                                                  :inputs inputs
                                                                  :observation-trace observation-trace)]
                                 [t s])))]
...
#?(:clj (defn importance-resampling-custom-proposal
...
#?(:clj (defn with-custom-proposal-attached
             ...
             (if (condition-for-use observations)
               (gen {:tracing-with t} [& args]
                 (let [custom-proposal
                       ...
                       [v tr2 p-score]
                       (mpg/infer-and-score :procedure orig-generative-function
                                            :inputs args
                                            :observation-trace proposed-trace)

                       [_ _ q-score]
                       (mpg/infer-and-score :procedure custom-proposal
                                            :inputs args
                                            :observation-trace proposed-trace)]
...
#?(:clj (defn symmetric-proposal-mh-step
...
#?(:clj (defn make-gaussian-drift-proposal
          [addresses width]
          (fn [current-trace]
            (reduce
             (fn [tr addr]
...
#?(:clj (defn gaussian-drift-mh-step
...
#?(:clj (defn custom-proposal-mh-step [& {:keys [model inputs proposal], :or {inputs []}}]
          (fn [current-trace]
            (let [[_ _ current-trace-score]               ;; Evaluate log p(t)
                  ...
                  (mpg/infer-and-score :procedure proposal
...
#?(:clj (defn make-gibbs-step
          [& {:keys [model address support inputs]
              :or {inputs []}}]
          (fn [current-trace]
            (let [log-scores
                  (map (fn [value]
                         (nth (mpg/infer-and-score :procedure model
                                                   :inputs inputs
                                                   :observation-trace
...
#?(:clj (def make-resimulation-proposal
...
#?(:clj (def resimulation-mh-move
          (gen [model inputs tr addresses]
            (let [[current-choices fixed-choices]
                  ...
                  [_ _ reverse-q]
                  (mpg/infer-and-score :procedure infer-and-score,
...
#?(:clj (defn check-bins-against-pdf
          [bins pdf]
          (let [n-samples
...
#?(:clj (defn check-samples-against-pdf
          [samples pdf nbins]
          (let [samples
                (vec (sort samples))
...
#?(:clj (defn report-on-elapsed-time [tag thunk]
